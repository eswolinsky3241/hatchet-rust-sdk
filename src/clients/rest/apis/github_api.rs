...
/// SNS event
pub async fn sns_update(configuration: &configuration::Configuration, tenant: &str, event: &str) -> Result<(), Error<SnsUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_tenant = tenant;
    let p_event = event;

    let uri_str = format!("{}/api/v1/sns/{tenant}/{event}", configuration.base_path, tenant=crate::clients::rest::apis::urlencode(p_tenant), event=crate::clients::rest::apis::urlencode(p_event));
