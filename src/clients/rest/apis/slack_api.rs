...
pub async fn slack_webhook_delete(
    configuration: &configuration::Configuration,
    slack: &str,
) -> Result<(), Error<SlackWebhookDeleteError>> {
    ...
    let uri_str = format!(
        ...
        slack = crate::clients::rest::apis::urlencode(p_slack)
    );
...
/// List Slack webhooks
pub async fn slack_webhook_list(
    configuration: &configuration::Configuration,
    tenant: &str,
) -> Result<models::SlackWebhookList200Response, Error<SlackWebhookListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_tenant = tenant;
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::SlackWebhookList200Response`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::SlackWebhookList200Response`"
                ))));
            }
        }
    } else {
